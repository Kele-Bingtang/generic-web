...
import { queryGenericUserRole } from "@/api/user";
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        meta: {
          title: (route: RouteConfig) => route.params.projectName + "",
          hideInMenu: true,
      ...
      {
        path: "service-col/:serviceName/:serviceId/:secretKey",
        name: "ServiceCol",
        component: "/serviceCol/index",
        meta: {
          title: (route: RouteConfig) => route.params.serviceName + "接口字段",
          hideInMenu: true,
          activeMenu: "/project/index",
        },
        beforeEnter: beforeEnterFun,
      },
      {
        ...
        meta: {
          title: (route: RouteConfig) => route.params.reportTitle + "报表",
          hideInMenu: true,
          activeMenu: "/project/index",
        },
        beforeEnter: beforeEnterFun,
      },
      {
        ...
        meta: { title: "成员管理", hideInMenu: true, activeMenu: "/project/index" },
        beforeEnter: beforeEnterFun,
      },
    ],
  },
...
function beforeEnterFun(to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) {
  const dataStore = useDataStore();
  const { secretKey } = to.params;
  if (!dataStore.project.secretKey) {
    dataStore.project.secretKey = secretKey as string;
  }
  if (!dataStore.userRole.code && secretKey !== from.params.secretKey) {
    queryGenericUserRole(secretKey as string)
      .then(res => {

  ...
  public constructor(config: AxiosRequestConfig) {
    ...
    this.service.interceptors.request.use(
      config => {
        ...
        if (dataStore.project.secretKey) config.headers["Secret-Key"] = dataStore.project.secretKey;
        return config;
      },
      (error: AxiosError) => {
        // TODO：请求的异常进行捕获处理
    ...
    this.service.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        // 根据响应的错误状态码，做不同的处理
        if (error.response) {
          if ((error.response.data as any)?.path === "/login") {
            message.error("身份异常！请您重新登录");
          } else checkStatus(error.response.status);
        }
